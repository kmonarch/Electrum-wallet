	...
	```
	 ...
	 3. 在此方法中，程序会根据本地的路径来判断是否已存在钱包，如存在则打开，不存在则创建新钱包
		...
		6. 此时程序会跳转到 `_init_.py` 文件中 执行
		 ...
		 9. 此后，程序跳转至`base_wizard.py` 中的 `def new(self):`方法，并在此方法中进行UI的初始化工作，初始化完成后，UI上回展示 让用户来选择希望创建的钱包类型。选择了其中一项并点击`next`按钮之后，程序会jin'ru 

<!--stackedit_data:
eyJoaXN0b3J5IjpbMTcyNDk4MDkxOSwxMzQ5MjUxNDkwLC0xMT
