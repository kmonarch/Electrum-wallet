 ...
 5. `daemon.py` 文件中的 `def init_gui(self, config, plugins):`
	...
	```
	 ...
	 2. 在此方法中，程序会根据本地的路径来判断是否已存在钱包，如存在则打开，不存在则创建新钱包
		...
		17. 程序再次回到`_init_.py` --> ` w = self.create_window_for_wallet(wallet)`
		18. ``_init_.py`` --> `def create_window_for_wallet(self, wallet):`
		19. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjY4MzE5OTU5LC0xODgwNDgxMzgwLDk5ND
czMDEzNiwxNjY0MTY4MzU2LC0xMjk1MjM4NzEsNjA1MDEyMzM5
LDU0MDUwMzMzMCwtMTYyODg3MjQ1OCwtMTAxNTc4MjU2LC05OT
k1OTE5NywtOTk5NTkxOTcsMjAxMDgzNzMxMCwtMjIyMjA0NTEx
LC0xMjQ2Mjc4MjM1LC0xNDIwMjQxODI5LDI0MjY2MzI5MSwxMT
g2MDkxNjc5LDEzNDkyNTE0OTAsLTExNTA2MzEzNTgsLTIxMjMz
MjY3ODBdfQ==
-->