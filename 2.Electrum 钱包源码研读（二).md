...
5. `daemon.py` 文件中的 `def init_gui(self, config, plugins):`
	1. 在此方法中会首先进行 'gui' 对象的实例化，实例化方法在 `_init_.py` 中的 `def __init__(self, config, daemon, plugins):`
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTUwMjYzMjY1LC0yMTgzMjY0NjMsMTM0Mj
IzMTY0NywxNDY1MTQ5OTAyXX0=
-->