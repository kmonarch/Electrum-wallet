...
5. `daemon.py` 文件中的 `def init_gui(self, config, plugins):`
	1. 在此方法中会首先进行 'gui' 对象的实例化
		...
		> 实例化方法在 `_init_.py` 中的 `def __init__(self, config, daemon, plugins):`
	3. 而后程序会返回`daemon.py` 文件中的 `def init_gui(self, config, plugins):` 去执行 `try: self.gui.main()`，即跳转到`_init_.py` 中的 `def main(self):`方法中，代码如下：
	```python
	def main(self):
        try:
            self.init_network()
	 ...
	 3. 在此方法中，程序会根据本地的路径来判断是否已存在钱包，如存在则打开，不存在则创建新钱包
		...
		```
		 ...
		 4. 此时程序会跳转到 `installwizard.py` 文件中执行实例化方法`def __init__(self, config, app, plugins, storage):`
			 1. 在这份代码中首先运行`BaseWizard.__init__(self, config, plugins, storage)` 改方法存在于`base_wizard.py`中 
		...
		6. 此时程序会跳转到 `_init_.py` 文件中 执行
		 ...
		 8. 在UI上点击了 `Next` 按钮之后，程序会跳转到 `installwizard.py`文件中的 `if self.storage.requires_split():` 而后继续顺序执行至：` self.run(action)`
		 9. 此后，程序跳转至`base_wizard.py` 中的 `def new(self):`方法，并在此方法中进行UI的初始化工作，初始化完成后，UI上回展示 让用户来选择希望创建的钱包类型。选择了其中一项并点击`next`按钮之后，程序会执行
		 ```python
		...
		在这里选择第一项`Standard wallet`,而后点击 `Next `
		 10. 随后程序会执行 `def choose_keystore(self):`方法来让用户选择创建种子或使用已有种子来恢复一个钱包，在这里选择`Create a new seed` ,而后点击 `Next `,程序会执行`base_wizard.py` 文件中的`def run(self, *args):` ，在条件判断
		 ```python
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTg1NTM5MDQzOCwtMjIyMjA0NTExLC0xMj
Q2Mjc4MjM1LC0xNDIwMjQxODI5LDI0MjY2MzI5MSwxMTg2MDkx
Njc5LDEzNDkyNTE0OTAsLTExNTA2MzEzNTgsLTIxMjMzMjY3OD
AsLTE4MDUwMjMyNDYsNTcxNDg3MDgzLDE1ODI4MzE2MjcsMjA3
NTY0OTYxMSwxMDAwMjYwOTIwLC0xOTI2Mjc5ODE0LC0xNjY4ND
M3NTU4LC01MDI2MzI2NSwtMjE4MzI2NDYzLDEzNDIyMzE2NDcs
MTQ2NTE0OTkwMl19
-->