 ...
 5. `daemon.py` 文件中的 `def init_gui(self, config, plugins):`
	...
	```
	 ...
	 3. 在此方法中，程序会根据本地的路径来判断是否已存在钱包，如存在则打开，不存在则创建新钱包
		...
		中进行一系列的判断，如果条件满足，则进行钱包的创建`` ---> ` def create_wallet(self):`
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwOTk0NzgwNDUsNjA1MDEyMzM5LDU0MD
UwMzMzMCwtMTYyODg3MjQ1OCwtMTAxNTc4MjU2LC05OTk1OTE5
NywtOTk5NTkxOTcsMjAxMDgzNzMxMCwtMjIyMjA0NTExLC0xMj
