...
5. `daemon.py` 文件中的 `def init_gui(self, config, plugins):`
	...
	2. 而后程序会返回`daemon.py` 文件中的 `def init_gui(self, config, plugins):` 去执行 `try: self.gui.main()`，即跳转到`_init_.py` 中的 `def main(self):`方法中.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY2MzM2NDE0OCwtNTAyNjMyNjUsLTIxOD
