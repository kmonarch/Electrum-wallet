	 ...
	 3. 在此方法中，程序会根据本地的路径来判断是否已存在钱包，如存在则打开，不存在则创建新钱包
		...
		```
		 ...
		 4. 此时程序会跳转到 `installwizard.py` 文件中执行实例化方法`def __init__(self, config, app, plugins, storage):`
			...
			```		
		5. 在`def __init__(self, config, app, plugins, storage):`方法中，程序会顺序执行到`self.show()` 此时 `gui`画面会展示出来。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExNTA2MzEzNTgsLTIxMjMzMjY3ODAsLT
E4MDUwMjMyNDYsNTcxNDg3MDgzLDE1ODI4MzE2MjcsMjA3NTY0
OTYxMSwxMDAwMjYwOTIwLC0xOTI2Mjc5ODE0LC0xNjY4NDM3NT
U4LC01MDI2MzI2NSwtMjE4MzI2NDYzLDEzNDIyMzE2NDcsMTQ2
NTE0OTkwMl19
-->