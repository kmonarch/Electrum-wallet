	...
	```
	 ...
	 3. 在此方法中，程序会根据本地的路径来判断是否已存在钱包，如存在则打开，不存在则创建新钱包
		...
		```
		 ...
		 ```
		 3. 在`start_new_window`方法中，程序会再次在本地路径中判断钱包是否存在，由于此时是程序的初始化，故本地不存在qian'bao
		

			

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTUxNTc4NTM5NywtMTkyNjI3OTgxNCwtMT
Y2ODQzNzU1OCwtNTAyNjMyNjUsLTIxODMyNjQ2MywxMzQyMjMx
NjQ3LDE0NjUxNDk5MDJdfQ==
-->