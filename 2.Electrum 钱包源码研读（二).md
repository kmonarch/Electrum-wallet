	...
	```
	 ...
	 3. 在此方法中，程序会根据本地的路径来判断是否已存在钱包，如存在则打开，不存在则创建新钱包
		...
		在这里选择第一项`Standard wallet`,而后点击 `Next `
		 10. 随后程序会执行 `def choose_keystore(self):`方法来让用户选择创建种子或使用已有种子来恢复一个钱包，在这里选择`Create a new seed` ,而后点击 `Next `,程序会执行`base_wizard.py` 文件中的

<!--stackedit_data:
eyJoaXN0b3J5IjpbMjQyNjYzMjkxLDExODYwOTE2NzksMTM0OT
I1MTQ5MCwtMTE1MDYzMTM1OCwtMjEyMzMyNjc4MCwtMTgwNTAy
MzI0Niw1NzE0ODcwODMsMTU4MjgzMTYyNywyMDc1NjQ5NjExLD
EwMDAyNjA5MjAsLTE5MjYyNzk4MTQsLTE2Njg0Mzc1NTgsLTUw
MjYzMjY1LC0yMTgzMjY0NjMsMTM0MjIzMTY0NywxNDY1MTQ5OT
AyXX0=
-->