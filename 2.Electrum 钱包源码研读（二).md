 ...
 5. `daemon.py` 文件中的 `def init_gui(self, config, plugins):`
	...
	```
	 ...
	 3. 在此方法中，程序会根据本地的路径来判断是否已存在钱包，如存在则打开，不存在则创建新钱包
		...
		13. 程序再次回到
		 >`_init_.py` --> `try: wallet = wizard.run_and_get_wallet(self.daemon.get_wallet)`
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTg1MjgwOTUwLDE2NjQxNjgzNTYsLTEyOT
UyMzg3MSw2MDUwMTIzMzksNTQwNTAzMzMwLC0xNjI4ODcyNDU4
LC0xMDE1NzgyNTYsLTk5OTU5MTk3LC05OTk1OTE5NywyMDEwOD
